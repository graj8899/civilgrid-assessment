...
| Measure | Value |
|---|---|
| Combined footprint, all 46 CIP projects | 0.21 km² |
| Median single footprint | ~1,580 m² (roughly 40 m × 40 m) |
| Smallest footprint | 4.3 m² |
| Share of the City of LA covered | 1 in 6,282 |
| **Expected chargers strictly inside, all 46 projects** | **~0.06** |
| Expected within 250 m | ~4 |
| Expected within 500 m | ~13 |
...
**How the numbers were derived**, since this is the first thing a sharp reviewer will poke at: sum `Shape_Area` across all 46 features, convert US survey feet to m², divide by the City of LA's 1,302 km², multiply by 384 chargers for a uniform-density expectation. Buffered figures add `perimeter × r + πr²` per footprint. Chargers do cluster in the same corridors where capital work happens, so real counts run above uniform density — but not by three orders of magnitude.
...
- **All 384 EV charger properties are null.** `slid`, `lat`, `lon`, `Date_Imported`, `TOOLTIP`, `NLA_URL` — every field, every feature. `OBJECTID` and the geometry are all there is, so a charger detail panel is impossible. The tooltip says so plainly rather than rendering blank rows.
- **Mixed geometry.** 43 `Polygon` + 3 `MultiPolygon` across 46 CIP features. A Polygon-only implementation silently drops chargers in the second lobe of three projects.
...
| State | `useState` + `useMemo` in `App.tsx` | 46 + 384 features, one screen, three-level tree |
...
**Cost.** 46 × 384 = 17,664 point-in-polygon tests, doubled when buffered. Single-digit milliseconds, so there is no bbox prefilter and no spatial index. The interface is index-agnostic — an rbush-backed `chargersInGeometry` drops in without touching a component.
...
| `selectedId` | `number \| null` | `PROJECTID`, not an object |
...
1. OSM tile layer
2. All 46 CIP footprints — muted grey fill, 1px outline, click to select, tooltip on hover
...
5. All 384 chargers — grey, r=3
6. Matched chargers — r=6, green when inside the footprint, blue when only within the radius

The dashed ring exists so the match logic is **visible** rather than implied, and the green/blue split makes the dual count legible on the map as well as in the panel. Only the selected project is buffered; buffering all 46 for display is wasted work.

**Implementation gotcha worth knowing before you start.** react-leaflet's `<GeoJSON>` does not re-read its `style` prop when props change — it snapshots on mount. Force a remount with `key={`cip-${selectedId}`}`. Remounting 46 polygons is a fraction of a millisecond and is simpler than an imperative `setStyle` pass over layers. Without this, selection appears to do nothing on the map, which is the most likely hour-long debugging detour in this build.
...
One file, `spatial.test.ts`. Hand-built geometry, no fixtures lifted from the real data. **25 tests, ~34 ms, no DOM.**
...
| `matchProject` | strict containment at radius 0 · ~100 m point lands in `nearby` not `inside` · buckets disjoint · `total` is the sum · 900 m excluded at 250 m · remote charger never matches · buffer present when radius > 0 · monotonic in radius · `projectId` is `PROJECTID` |
| `buildMatches` | keyed by `PROJECTID` · includes zero-match projects so the list still renders them · each project scoped to its own chargers · empty project list |
...
- **The MultiPolygon case earns its keep** even though it passes trivially with `booleanPointInPolygon`. It catches the hand-rolled ring walk, which is the tempting optimisation and would silently break 3 of the 46 real projects.
...
| Spatial index (rbush) | 17k checks, milliseconds | County-wide, thousands of footprints |
| Web worker | Discrete radius stops remove the jank case | Continuous slider over much larger data |
| Charger detail panel | All 384 records have null attributes | Port, power and operator data arrive |
...
| Question | Answer |
|---|---|
| How do you define "within that area"? | Adjustable radius, 500 m default, both counts always shown. Strict containment yields ~0.06 expected matches across all 46 projects — I derived that from footprint area against charger density, and verified the `Shape_Area` unit against a known bridge length. A literal reading ships an empty tool. |
...
| Why Turf over hand-rolled geometry? | MultiPolygon and ring-winding edge cases. Three of 46 features are MultiPolygon and a hand-rolled version drops them silently — there is a test for exactly that. |
| How would it scale? | Bbox prefilter → rbush → worker → PostGIS. `chargersInGeometry` is index-agnostic, so the first two are drop-in. |
| What is wrong with the data? | All 384 charger records have null attributes, so the tool can locate chargers but not rank them. `Shape_Area` is in a projected CRS and misleads if read as degrees. `StartDate` duplicates `ConsStartDate`. I would request port count, power rating and operator. |
...
- [ ] All 46 footprints and 384 chargers render
- [ ] Selecting from the map and from the list produce the same result
- [ ] MultiPolygon projects match chargers correctly
- [ ] Radius control changes the counts *and* the dashed ring
- [ ] Zero-match and no-selection states read as intentional, not broken
- [ ] `npm run build` clean, `npm test` green, no console errors
- [ ] README covers setup, the §1 decision, the §2 data findings, and the §12 cuts
- [ ] Repo is private, with **@Ujjwel Balwal** added as a collaborator (not a public fork — the assessment repo already has public forks, and a fork would expose your solution)
- [ ] Reply to Shyam confirming completion and offering to walk through it
